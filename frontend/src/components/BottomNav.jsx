...
export default function BottomNav({ activeTab, setActiveTab }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 flex h-16 items-center justify-around border-t border-whisper bg-white px-2 dark:border-neutral-800 dark:bg-[#1f1f1f] md:hidden pb-safe">
      {tabs.map((tab) => (
        <button
          key={tab.id}
